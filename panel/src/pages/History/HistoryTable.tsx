...
function HistoryRow({ action, modalOpener }: HistoryRowProps) {
    ...
    } else if (action.type === 'ban' || action.type === 'wagerblacklist') {
    ...
    } else if (action.revocation.status === 'not_revoked') {
        if (action.banExpiration) {
    ...
    return (
        <TableRow
            ...
            className={cn(
                'cursor-pointer',
                action.revocation.status === 'pending' && 'bg-amber-100 dark:bg-amber-950 hover:bg-amber-200 dark:hover:bg-amber-900',
            )}
        >
            <TableCell className={cn(
                'w-[10.4rem] border-r p-0',
                action.revocation.status === 'approved' && 'opacity-40'
            )}>
